...
/**
 * Walks the full Esther's Rescue of Her People adventure end to end. It's
 * locked until Noah's Ark, David and Goliath, Good Samaritan, and Daniel are
 * completed — and this device's save data persists real state across test
 * runs — so this test completes all four prerequisites itself rather than
 * assuming they're already done, to stay deterministic regardless of what
 * ran before it (same pattern as DanielFlowTest).
 */
class EstherFlowTest {
    ...
    @Test
    fun completingEsther_awardsStarsAndUnlocksJerichoOnTheWorldMap() {
        ...
        // World Map -> Esther's Rescue of Her People (now unlocked).
        composeTestRule.onNodeWithText(activity.getString(R.string.chapter_esther_title)).performClick()

        // Scene 1: Intro.
        composeTestRule.onNodeWithText(continueLabel).performClick()

        // Scene 1b: The King's Trouble context.
        composeTestRule.onNodeWithText(continueLabel).performClick()

        // Scene 1c: Haman's Anger context.
        composeTestRule.onNodeWithText(continueLabel).performClick()

        // Scene 2: Choice — Esther's decision, flavor-only.
        composeTestRule.onNodeWithText(activity.getString(R.string.esther_choice_option_1)).performClick()
        composeTestRule.onNodeWithText(continueLabel).performClick()

        // Scene 2b: The Golden Scepter context.
        composeTestRule.onNodeWithText(continueLabel).performClick()

        // Scene 3: The Two Banquets — wait, wait, then speak.
        ...
        // Scene 3b: The Truth Revealed context.
        composeTestRule.onNodeWithText(continueLabel).performClick()
        ...
        // Back on the World Map: Esther completed, The Battle of Jericho unlocked.
