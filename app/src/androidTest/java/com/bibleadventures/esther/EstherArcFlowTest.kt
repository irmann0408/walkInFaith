...
/**
 * Walks the full 5-chapter Esther arc end to end, asserting explicitly that
 * each chapter's completion unlocks the very next one on the World Map —
 * more thorough than JerichoFlowTest's own walkthrough of this same arc
 * (which only implicitly relies on the chain being correct). Kept as one
 * consolidated test rather than 5 separate FlowTest files: every existing
 * FlowTest re-derives its full prerequisite chain from scratch since device
 * state can't be assumed clean, and 5 separate files would each redundantly
 * re-walk Noah's Ark, David & Goliath, Good Samaritan, and Daniel just to
 * test one chapter — a real combinatorial cost this app hadn't hit before
 * this arc.
 */
class EstherArcFlowTest {
    ...
    @Test
    fun completingAllFiveEstherChapters_unlocksEachNextChapterAndFinallyJericho() {
        ...
        composeTestRule.onNodeWithText(activity.getString(R.string.badge_humble_trust_title)).assertExists()
        ...
        composeTestRule.onNodeWithText(activity.getString(R.string.chapter_esther_secret_plot_title)).assertExists()

        // --- Chapter 2: The Secret Plot ---
        ...
        composeTestRule.onNodeWithText(continueLabel).performClick()
        composeTestRule.onNodeWithText(activity.getString(R.string.reward_title)).assertExists()
        composeTestRule.onNodeWithText(activity.getString(R.string.badge_watchful_ears_title)).assertExists()
        composeTestRule.onNodeWithText(activity.getString(R.string.action_return_to_map)).performClick()
        scrollToChapterOnWorldMap(activity.getString(R.string.chapter_esther_threat_title))
        composeTestRule.onNodeWithText(activity.getString(R.string.chapter_esther_threat_title)).assertExists()
        ...
        composeTestRule.onNodeWithText(continueLabel).performClick()
        composeTestRule.onNodeWithText(activity.getString(R.string.reward_title)).assertExists()
        composeTestRule.onNodeWithText(activity.getString(R.string.badge_faithful_messenger_title)).assertExists()
        composeTestRule.onNodeWithText(activity.getString(R.string.action_return_to_map)).performClick()
        scrollToChapterOnWorldMap(activity.getString(R.string.chapter_esther_brave_approach_title))
        composeTestRule.onNodeWithText(activity.getString(R.string.chapter_esther_brave_approach_title)).assertExists()

        // --- Chapter 4: The Brave Approach ---
        ...
        composeTestRule.onNodeWithText(activity.getString(R.string.chapter_esther_banquets_rescue_title)).assertExists()

        // --- Chapter 5: The Banquets & Rescue ---
        ...
        composeTestRule.onNodeWithText(continueLabel).performClick()
        composeTestRule.onNodeWithText(activity.getString(R.string.reward_title)).assertExists()
        composeTestRule.onNodeWithText(activity.getString(R.string.badge_bold_voice_title)).assertExists()
        composeTestRule.onNodeWithText(activity.getString(R.string.action_return_to_map)).performClick()

        // Closing the loop: the whole arc unlocks Jericho.
    ...
    /** Walks Daniel and the Lions end to end (mirrors DanielFlowTest) so Esther: The New Queen unlocks. */
